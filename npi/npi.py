...
if __name__ == "__main__":
    ...
    #           General
    # =================================
    parser.add_argument("--key", type=str, nargs='+', default=None, help=key_pattern_rules())
    ...
    if args.fp is not None:
        arrays_dict = get_data_dictionary(args.fp, args.tag)

        arrays_dict, _ = filter_data_by_key(arrays_dict, args.key) if args.key is not None else (arrays_dict, 0)

        arrays_dict = dict(sorted(arrays_dict.items(), key=lambda item: item[0]))  # sorting keys

        arrays_dict = transform_data(arrays_dict)

        view_data(arrays_dict)
        arrays_dict = stack_data(arrays_dict)
        find_data(arrays_dict)
        plot_data(arrays_dict)
        save_data(arrays_dict)
        metrics(arrays_dict)

        if args.rename is not None or args.remove is not None:
            if args.saveas is None:
                args.saveas = args.fp
            dict_keys = list(arrays_dict.keys())
            if args.rename is not None:
                num_rename_args = len(args.rename)
                assert num_rename_args % 2 == 0, "Number of rename args must be even with source and target key pairs"
                pairs = list(zip(args.rename[0::2], args.rename[1::2]))
                for (src_key, dst_key) in pairs:
                    assert src_key in dict_keys, f"{src_key} not found in {dict_keys}"
                    print(f"Renaming key {src_key} to {dst_key}")
                    arrays_dict[dst_key] = arrays_dict.pop(src_key)
            else:
                del_keys = args.remove
                for del_key in del_keys:
                    arrays_dict.pop(del_key) if del_key in dict_keys else print(f"{del_key} not found in {dict_keys}")
            save_data(arrays_dict)
    ...
    if args.append is not None:
        ...
        source, destination = args.append[0:-1], args.append[-1]
        assert destination.endswith(".npz"), NotImplementedError("Only .npz destination files are supported")
        target = dict(np.load(destination))
        for a_id in source:
            a_fp, key_id = a_id.split("::")
            key_id_splits = key_id.split("@")
            if len(key_id_splits) == 1:
                new_key_id = key_id
            elif len(key_id_splits) == 2:
                key_id, new_key_id = key_id_splits
            else:
                raise ValueError("Invalid key_id: " + key_id)
            arrays_dict = get_data_dictionary(a_fp, args.tag)
            # TODO: Need to make the arrays_dict evaluation generic with specific pattern
            target[new_key_id] = arrays_dict[key_id]
        assert args.saveas is not None, "Please specify a destination file to save the appended data"
        np.savez_compressed(args.saveas, **target)
